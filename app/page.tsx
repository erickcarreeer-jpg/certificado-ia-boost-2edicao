...
import ReissueCertificate from "@/components/ReissueCertificate"
import Image from "next/image"

type Stage = "intro" | "verify" | "quiz" | "checking" | "passed" | "failed" | "reissue"
...
export default function Home() {
  ...
  const [reissueData, setReissueData] = useState<ReissueData | null>(null)
  ...
  // Called when email is verified and student hasn't passed yet
  function handleVerified(name: string, email: string) {
    setStudentName(name)
  ...
  // Called when student already passed the quiz previously
  function handleAlreadyPassed(data: ReissueData) {
    setReissueData(data)
    setStage("reissue")
    scrollToTop()
  }
    ...
    async (answers: Record<number, string>) => {
      ...
      try {
        const res = await fetch("/api/check-answers", {
          ...
          body: JSON.stringify({ answers, email: studentEmail, name: studentName }),
        })
  ...
  // Retry quiz (failed) — re-use same email/name, no re-verify
  function retryQuiz() {
    setResult(null)
    setShuffled(shuffleQuestions(questions))
    setStage("quiz")
    ...
    >
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div
              ...
              aria-label="Evento por Gilberto Prado, Leandro Rezende e Rafael Coronel"
            >
              <span className="text-xs" aria-hidden="true" style={{ color: "rgba(255,255,255,0.45)" }}>
                Evento por
              </span>
          ...
          <section className="relative overflow-hidden pt-32 pb-0">
            ...
            <div className="relative max-w-6xl mx-auto px-6 text-center">
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow = "0 0 0 1px rgba(255,255,255,0.4), 0 12px 48px rgba(114,26,231,0.45)"
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.boxShadow = "0 0 0 1px rgba(255,255,255,0.2), 0 8px 40px rgba(114,26,231,0.3)"
                }}
              >
                Iniciar prova
                ...
                {[
                  {
                    ...
                    desc: "Digite seu nome e o e-mail usado na inscrição do workshop para verificar sua participação.",
                  },
                  {
                    num: "02",
                    icon: "📝",
                    title: "Responda a prova",
                    desc: "10 questões sobre o conteúdo do Workshop IA Boost 2ª Edição. Acerte 7 ou mais para passar.",
                  },
                  {
                    num: "03",
                    icon: "🏆",
                    title: "Emita o certificado",
                    desc: "Após a aprovação seu certificado sai na hora em PNG ou PDF com seu nome!",
                  },
                ].map((step, i) => (
                  <div
                    key={step.num}
                    className="p-8 flex flex-col gap-4"
                    style={{ backgroundColor: i === 1 ? "rgba(114,26,231,0.06)" : "rgba(255,255,255,0.02)" }}
                  >
      ...
      {stage !== "intro" && (
        <div
          ref={mainContentRef}
          tabIndex={-1}
          className="max-w-3xl mx-auto px-6 pt-28 pb-16 outline-none"
        >
          {stage === "verify" && (
            <VerifyStudent
              onVerified={handleVerified}
              onAlreadyPassed={handleAlreadyPassed}
            />
          )}
          ...
          {stage === "reissue" && reissueData && (
            <ReissueCertificate
              email={reissueData.email}
              initialName={reissueData.name}
              score={reissueData.score}
              correct={reissueData.correct}
              total={reissueData.total}
              onRetry={retry}
            />
          )}
        </div>
